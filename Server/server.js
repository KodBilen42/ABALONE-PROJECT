...
class game_Session {
  ...
  process_data(message = "") {
    if (message == null) return this.my_board.return_data()
    let balls = []
...
wss.on("connection", function connection(ws, req) {
  ...
  ws.on("message", (data) => {
    ...
    if (data === "session_request") {
    ...
    } else if (data.slice(0, 12) === "move_request") {
      //move information from browser
      ...
      for (let i = 0; i < sessions.length; i++) {
        if (sessions[i].includes(ws)) {
          ...
          if (
            (sessions[i][0].turn_color === "W" && index === 2) ||
            (sessions[i][0].turn_color === "R" && index === 1)
          )
            turn_check = true
        }
      }
